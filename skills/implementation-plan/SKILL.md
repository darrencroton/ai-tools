...
- **Mode B (Autonomous)** — when the plan is well-isolated and you want to step away. The orchestrator runs all remaining slices, delegates hostile drift-audit and independent review per slice, recovers from findings itself, and commits each slice that clears all gates. You assess at the end.
