...
4. Output only the markdown summary — no preamble, commentary, or surrounding text. If the user specifies a file path, write to that file; otherwise print in the conversation. If the user asks to write to a file but gives no path, derive the filename as **`Author(s) - Year - Title.md`** (1 author: surname only; 2 authors: `A and B`; 3+: `A et al`; title truncated to 80 chars, unsafe characters removed) and write to the current directory.
