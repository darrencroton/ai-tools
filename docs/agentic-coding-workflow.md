...
- report the authorization gate before normal quality summary
...
Call `ai-orchestrator` when the work benefits from delegation, independent review, long-running checks, or another model's perspective.
...
Only after that passes should normal code review start.
...
Call `code-review` after the authorization gate, or for standalone reviews.

When a frozen contract exists, `code-review` should start with the authorization gate, then review correctness, edge cases, tests, error handling, maintainability, and any domain-specific risks.
